...
def submitForm(driver, year, state, district, crop, fertilizer, downloadDir):
    """
    ...
    :return:
    """
    new_file = os.path.join(downloadDir, 'districttable4a - ' + year + '-' + state + '-' + district + '-' + crop +
                            '-' + fertilizer + '.xlsx')
    ...
    old_file = os.path.join(downloadDir, 'districttable4a.xlsx')
...
def downloadFiles(index_year, rootDir, state_start, district_start, crop_start, fertilizer_start):
    """
    ...
    for index_state in range(state_start, num_options_state):
        ...
        for index_district in range(district_loop_start, num_options_district):
            try:
                ...
                cropping_pattern_table_index = findIndexByText(dropdown_tables, 'TABLE4A-USAGE OF DIFFERENT FERTILIZERS FOR DIFFERENT CROPS')
                dropdown_tables.find_elements_by_tag_name('option')[cropping_pattern_table_index].click()

                dropdown_crops = driver.find_element_by_id("_ctl0_ContentPlaceHolder2_crops1")
                ...
                for index_crops in range(crop_loop_start, num_options_crops):
                    ...
                    dropdown_fertilizer = driver.find_element_by_id("_ctl0_ContentPlaceHolder2_fertilizers")
                    num_options_fertilizer = len(dropdown_fertilizer.find_elements_by_tag_name('option'))
                    if index_crops == crop_start:
                        fertilizer_loop_start = fertilizer_start
                    for index_fertilizer in range(fertilizer_loop_start, num_options_fertilizer):
                        ...
                        dropdown_input = [('_ctl0_ContentPlaceHolder2_ddlYear', index_year),
                                          ...
                                          ('_ctl0_ContentPlaceHolder2_ddlTables', cropping_pattern_table_index),
                                          ('_ctl0_ContentPlaceHolder2_crops1', index_crops),
                                          ('_ctl0_ContentPlaceHolder2_fertilizers', index_fertilizer)]
                        ...
                        try:
                            ...
                            submitForm(driver, str(index_year), str(index_state), str(index_district), str(index_crops),
                                       str(index_fertilizer), downloadDir)
                            global _state_start
                            _state_start = index_state
                            global _district_start
                            _district_start = index_district
                            global _crop_start
                            _crop_start = index_crops
                            global _fertilizer_start
                            _fertilizer_start = index_crops + 1
                            log.info('successfully downloaded configuration: y' + str(index_year) + '-s' +
                                     ...
                                     + '-c' + str(index_crops) + '-f' + str(index_fertilizer))
                            ...
                            if (options != None):
                                ...
                                log.error('There was an error while submitting the form for options:\n' +
                                          ...
                                          'Fertilizer index: ' + str(index_fertilizer) + '\n')
                            log.debug(e)
                            for i in range(0, 2):
                                ...
                                try:
                                    ...
                                    submitForm(driver, str(index_year), str(index_state), str(index_district),
                                               str(index_crops), str(index_fertilizer), downloadDir)
                                    _state_start = index_state
                                    _district_start = index_district
...
if not rootDir:
    rootDir = os.path.join(os.getcwd(), 'fertilizer')
...
config = input('Specify the indices of the state, district, crop, and fertilizer separated by commas (optional): ')
if not config:
    _state_start = 0
    _district_start = 0
    _crop_start = 0
    ...
    _fertilizer_start = config.split(',')[3]


while True:
    try:
        downloadFiles(int(year), rootDir, int(_state_start), int(_district_start), int(_crop_start), int(_fertilizer_start))
        break
    except Exception as e:
        print(e)
