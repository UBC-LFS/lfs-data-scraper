...
def submitForm(driver, year, state, district, downloadDir):
    ...
    new_file = os.path.join(downloadDir,
                            'districttable6A - ' + year + '-' + state + '-' + district  + '.xlsx')
    ...
    old_file = os.path.join(downloadDir, 'districttable6A.xlsx')
    ...
    for index_state in range(state_start, num_options_state):
        ...
        for index_district in range(district_loop_start, num_options_district):
            try:
                ...
                tenancy_table_index = findIndexByText(dropdown_tables, 'TABLE6A-CATTLE POSSESSED BY OPERATION HOLDERS')
...
if not rootDir:
    rootDir = os.path.join(os.getcwd(), 'cattle')
