...
def submitForm(driver, year, state, district, downloadDir):
    ...
    new_file = os.path.join(downloadDir,
                            'districttable5H - ' + year + '-' + state + '-' + district  + '.xlsx')
    ...
    old_file = os.path.join(downloadDir, 'districttable5H.xlsx')
    ...
    for index_state in range(state_start, num_options_state):
        ...
        for index_district in range(district_loop_start, num_options_district):
            try:
                ...
                tenancy_table_index = findIndexByText(dropdown_tables, 'TABLE5H-USAGE OF PESTICIDES FOR DIFFERENT CROPS')
...
if not rootDir:
    rootDir = os.path.join(os.getcwd(), 'pesticides')
