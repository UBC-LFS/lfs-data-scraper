...
def submitForm(driver, year, state, district, downloadDir):
    """
    ...
    new_file = os.path.join(downloadDir,
                            'districttable6C - ' + year + '-' + state + '-' + district  + '.xlsx')
    ...
    old_file = os.path.join(downloadDir, 'districttable6C.xlsx')
    ...
    for index_state in range(state_start, num_options_state):
        ...
        for index_district in range(district_loop_start, num_options_district):
            try:
                ...
                tenancy_table_index = findIndexByText(dropdown_tables, 'TABLE6C-OTHER LIVESTOCK POSSESSED BY OPERATION HOLDERS')
...
if not rootDir:
    rootDir = os.path.join(os.getcwd(), 'other_livestock')
...
while True:
    try:
        downloadFiles(int(year), rootDir, int(_state_start), int(_district_start))
        break
    except Exception as e:
        print(e)
